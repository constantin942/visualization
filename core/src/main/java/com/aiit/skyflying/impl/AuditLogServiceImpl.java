...
import com.aiit.skyflying.common.dao.MsConfigDao;
...
import com.aiit.skyflying.dao.MsDmsAuditLogDao;
import com.aiit.skyflying.dao.MsScheduledTaskDao;
import com.aiit.skyflying.service.AuditLogService;
...
public class AuditLogServiceImpl implements AuditLogService {
    ...
    public ServerResponse<String> batchProcessDmsAuditLog(List<ListSQLExecAuditLogResponse.SQLExecAuditLog> listSqlExecAuditLogList) {
        ...
        try {
            List<MsDmsAuditLogDo> msDmsAuditLogDoList = new LinkedList<>();
            for (ListSQLExecAuditLogResponse.SQLExecAuditLog sqlExecAuditLog : listSqlExecAuditLogList) {
                ...
                // 组装MsDmsAuditLogDo实例
                getMsDmsAuditLogDo(msSql, msSchemaName, sqlExecAuditLog, msDmsAuditLogDoList);
            }
            size = msDmsAuditLogDoList.size();
            if (0 < size) {
                // 将来自DMS的SQL审计日志信息存储到专门存储DMS审计日志的表中；2022-05-30 16:36:22
                msDmsAuditLogDao.insertSelectiveBatchNoSqlInsightDbUserName(msDmsAuditLogDoList);

                // 异常检测和统计；2022-10-09 09:16:02
                anomalyDetectionStatistics(msDmsAuditLogDoList);
            }
        } catch (Exception e) {
    ...
    private void anomalyDetectionStatistics(List<MsDmsAuditLogDo> msDmsAuditLogDoList) {
        try {
            ...
            for (MsDmsAuditLogDo msDmsAuditLogDo : msDmsAuditLogDoList) {
                ...
                msSegmentDetailDo.setDbInstance(msDmsAuditLogDo.getMsSchemaName());
                msSegmentDetailDo.setServiceCode(msDmsAuditLogDo.getSqlSource());
    ...
    private void getMsDmsAuditLogDo(String msSql, String msSchemaName, ListSQLExecAuditLogResponse.SQLExecAuditLog sqlExecAuditLog, List<MsDmsAuditLogDo> list) {
        ...
        try {
            if (!Const.SQL_TYPE_NONE.trim().equals(sqlType1) && StringUtil.isNotBlank(sqlType1) && !Const.FAIL.equals(sqlExecAuditLog.getExecState())) {
                ...
                String key = instanceName + Const.POUND_KEY + msSchemaName + Const.POUND_KEY + tableName;
