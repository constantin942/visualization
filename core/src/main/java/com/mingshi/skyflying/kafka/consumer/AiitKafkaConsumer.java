...
import com.mingshi.skyflying.domain.Ref;
...
public class AiitKafkaConsumer {
  ...
  private void useNoReactorModel(ConsumerRecord<String, Bytes> record) {
    try {
      ...
      Segment segment = setUserNameAndToken(spanList);
  ...
  private void reorganizingSpans(Segment segment, List<Span> spanList) {
   ...
   * @return void
   **/
  private boolean setUserNameByToken(Segment segment) {
   ...
   * @Param [segment]
   **/
  private void insertUserNameAndToken(Segment segment) {
  ...
  private Segment setUserNameAndToken(List<Span> spanList) {
  ...
  public Segment convert(SegmentObject segmentObject) {
