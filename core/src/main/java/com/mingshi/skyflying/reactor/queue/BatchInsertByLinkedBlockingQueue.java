...
public class BatchInsertByLinkedBlockingQueue {
  ...
  public BatchInsertByLinkedBlockingQueue(Integer localStatisticsThreadCount, Integer flushToRocketMQInterval, SegmentDao segmentDao, UserTokenDao userTokenDao) {
    ...
    for (Integer integer = 0; integer < localStatisticsThreadCount; integer++) {
      IoThread ioThread = new IoThread(linkedBlockingQueue, flushToRocketMQInterval, segmentDao, userTokenDao);
  ...
  public static LinkedBlockingQueue getLinkedBlockingQueue(Integer localStatisticsThreadCount, Integer flushToRocketMQInterval, SegmentDao segmentDao, UserTokenDao userTokenDao) {
    if (null == linkedBlockingQueue) {
      synchronized (BatchInsertByLinkedBlockingQueue.class) {
        if (null == linkedBlockingQueue) {
          log.info("获取单例LinkedBlockingQueue。");
          new BatchInsertByLinkedBlockingQueue(localStatisticsThreadCount, flushToRocketMQInterval, segmentDao, userTokenDao);
        }
      }
    }
